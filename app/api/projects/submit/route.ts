    ...
    <html>
      ...
      <body style="font-family: Arial, sans-serif; color: #111827; line-height:1.6; max-width: 760px; margin:0 auto;">
        <div style="background: linear-gradient(135deg, #FACC15 0%, #F59E0B 100%); padding: 24px; border-radius: 8px 8px 0 0;">
          <h1 style="margin:0; color:#111; font-size: 22px;">New Project Submission</h1>
          <p style="margin:6px 0 0 0; color:#111;">Archalley Project Submit Form</p>
        </div>
        ...
        <div style="font-size:12px; color:#6b7280; padding: 12px 24px; border:1px solid #e5e7eb; border-top:0; border-radius:0 0 8px 8px;">
          <p style="margin:0;">Submitted via Archalley Projects | ${new Date().toISOString()}</p>
        </div>
      </body>
    </html>
  `
...
export async function POST(request: NextRequest) {
  try {
    ...
    const firmInstitute = form.get("firmInstitute") as string
    ...
    const emailData: Record<string, string> = {
      ...
      firmInstitute: firmInstitute || "Not provided",
    ...
    const subject = `Project Submission: ${name}`
    const html = formatSubmissionEmail(emailData)
    const text = `New project submission\nName: ${name}\nEmail: ${email}\nContact: ${contactNumber || "N/A"}`
    ...
    console.log(`✅ Project submission email sent successfully. ID: ${info.messageId}`)
