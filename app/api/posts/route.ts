...
export async function POST(request: Request) {
  ...
  try {
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      // Generate title from content for all posts (including image posts)
      const autoGeneratedTitle = generateTitleFromContent(data.content)
      
      // Create the post with enhanced categorization and auto-generated title
      const newPost = await tx.post.create({
        data: {
          id: crypto.randomUUID(),
          title: autoGeneratedTitle, // Auto-generate title from content
 ...
 * Creates a meaningful title from the post description/content
 * 
 * @param content - Post content to generate title from
