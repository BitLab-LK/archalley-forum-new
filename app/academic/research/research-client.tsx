...
export default function ResearchClient({ 
  ...
  const fallbackProjects = initialProjects.length === 0 ? [
    {
      id: 1,
      title: { rendered: "Research & Publications Coming Soon" },
      excerpt: { rendered: "We're currently updating our research and publications archive. Please check back soon for our latest academic work." },
      content: { rendered: "Our research papers, academic publications, and studies showcase will be available soon with detailed abstracts and access to full publications." },
      featured_media: 1,
  ...
  const loadMoreProjects = async () => {
    ...
    try {
      ...
      const newProjects = await getResearchPosts(newPage, 20)
  ...
  const displayCategories = categories.length > 0 ? categories : [
    { id: 1, name: "Research & Publications", slug: "research", description: "Academic research and publications", count: 1 }
  ]
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center mb-16">
          ...
          <p className="text-xl text-muted-foreground max-w-3xl mx-auto mb-8">
            Explore our academic research, publications, papers, and studies in architecture, 
            design, and related fields. Access our latest findings and scholarly contributions.
          </p>
        </div>
        ...
        {displayProjects.length === 0 && !isLoading && (
          <div className="text-center py-12">
            <h3 className="text-lg font-medium text-muted-foreground mb-2">
              {searchTerm ? 'No research found' : 'No research available'}
            </h3>
            <p className="text-muted-foreground">
              {searchTerm 
                ? 'Try adjusting your search terms or filters'
                : 'Research papers and publications will be displayed here once they are available.'
              }
            </p>
          </div>
        )}
        ...
        {filteredProjects.length === 0 && searchTerm && hasRealData && !isLoading && (
          <div className="text-center py-12">
            <h3 className="text-lg font-medium text-muted-foreground mb-2">
              No research found for "{searchTerm}"
            </h3>
            <p className="text-muted-foreground">
              Try adjusting your search terms or clear filters to see all research
            </p>
            <Button 
              variant="outline" 
              className="mt-4"
            ...
            <div className="space-y-8">
              ...
              <div className="bg-white dark:bg-gray-800 rounded-lg border border-gray-100 dark:border-gray-700 shadow-sm p-1">
                <AdBannerComponent 
                  size="320x320" 
                  className="w-full" 
                  positionId="sidebar-square-research"
                  autoRotate={true}
                  rotationInterval={30}
                  showLabel={false}
                />
