...
const CATEGORY_SLUG_MAP: Record<string, string> = {
  ...
  'student-projects': 'student-project', // Support old slug
}
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  if (!wpCategory) {
    ...
    return {
      ...
      description: `Explore ${wpCategoryAlt.name} and academic work.`,
    }
  }
  
  return {
    title: `${wpCategory.name} | Archalley`,
    description: `Explore ${wpCategory.name} and academic work.`,
  }
}

export default async function AcademicCategoryPage({ params, searchParams }: PageProps) {
  const { category } = await params
  ...
  const basePath = `/academic/${category}`
