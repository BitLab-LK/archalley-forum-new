...
async function checkPayHerePayments() {
  try {
    ...
    if (payherePayments.length === 0) {
      ...
      // Group by status
      const byStatus = {
        COMPLETED: payherePayments.filter((p: any) => p.status === 'COMPLETED'),
        PENDING: payherePayments.filter((p: any) => p.status === 'PENDING'),
        FAILED: payherePayments.filter((p: any) => p.status === 'FAILED'),
      };
      ...
      // Calculate revenue
      const totalRevenue = byStatus.COMPLETED.reduce((sum: number, p: any) => sum + p.amount, 0);
      console.log(`💰 Total Revenue from PayHere: LKR ${totalRevenue.toLocaleString()}\n`);

      console.log('═'.repeat(80));
      console.log('\n📋 Recent PayHere Payments:\n');

      payherePayments.slice(0, 10).forEach((payment: any, index: number) => {
