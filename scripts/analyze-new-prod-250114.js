...
async function analyzeMerchant250114() {
  try {
    ...
    console.log('🆕 NEW PRODUCTION MERCHANT: 250114');
    console.log('='.repeat(80));
    console.log(`\nTotal Payments: ${payments.length}`);
    console.log(`Total Amount: ${payments.reduce((sum, p) => sum + p.amount, 0).toLocaleString()} LKR\n`);
    
    if (payments.length === 0) {
      console.log('❌ No payments found for merchant 250114\n');
      return;
    }
    ...
    // Warning if no completed payments
    if (completedPayments.length === 0) {
      console.log('\n' + '⚠️'.repeat(40));
      console.log('⚠️  WARNING: NO COMPLETED PAYMENTS!');
      console.log('⚠️  All payments are in PENDING status');
      console.log('⚠️  This suggests webhook/integration issues');
      console.log('⚠️  PayHere notifications may not be reaching the server');
    ...
    console.log('\n' + '='.repeat(80));
    console.log('📊 SUMMARY - MERCHANT 250114');
    console.log('='.repeat(80));
